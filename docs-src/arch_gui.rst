...
Common components are implemented in :ref:`mahos.core.gui` package.
...
The :class:`QBasicMeasClient <mahos.core.gui.client.QBasicMeasClient>` can be used as `Qt-version` client of :class:`BasicMeasNode <mahos.core.meas.common_meas.BasicMeasNode>`.
...
It is important that :class:`QBasicMeasClient <mahos.core.gui.client.QBasicMeasClient>` does all the mahos communication stuffs (:term:`Req-Rep` and :term:`Pub-Sub`); it `converts Qt-communication to mahos-communication` in other words.
As a result, ``IVCurveWidget`` don't have to care about mahos communications and can focus on using `signals` and `slots` provided by :class:`QBasicMeasClient <mahos.core.gui.client.QBasicMeasClient>`.
