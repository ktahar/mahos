...
Fundamental features are implemented in :ref:`mahos.core.inst` package.

InstrumentServer
----------------

The core of inst layer is :class:`InstrumentServer <mahos.core.inst.server.InstrumentServer>` node.
...
An instrument driver is implemented as a subclass of :class:`Instrument <mahos.core.inst.instrument.Instrument>`.
The instrument should provide :ref:`instrument-api` by overriding methods like ``start()``.

InstrumentOverlay
-----------------

:class:`InstrumentOverlay <mahos.core.inst.overlay.overlay.InstrumentOverlay>` is a mechanism to define `virtual instrument` using multiple instruments.
The role is similar to :doc:`arch_meas`; however, overlay works on the same process / thread with each instrument and has direct reference to the :class:`Instrument <mahos.core.inst.instrument.Instrument>` instances.
...
However, one cannot understand the usage of :meth:`set <mahos.core.inst.instrument.Instrument.set>`,
:meth:`get <mahos.core.inst.instrument.Instrument.get>`, and :meth:`configure <mahos.core.inst.instrument.Instrument.configure>` from their signatures.
...
:class:`InstrumentInterface <mahos.core.inst.interface.InstrumentInterface>`.
...
Instrument can report available parameters by returning a :class:`ParamDict <mahos.core.msgs.param_msgs.ParamDict>`
for :meth:`get_param_dict() <mahos.core.inst.instrument.Instrument.get_param_dict>` call.
The users can modify the parameters in it, and send it back through :meth:`configure() <mahos.core.inst.instrument.Instrument.configure>`.
