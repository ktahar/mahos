...
The most `practical` way may be to try :ref:`to build your own system <next-step-build>`.
...
If you want to build your own system, the first step might be preparing the :class:`Instrument <mahos.inst.instrument.Instrument>` class for your instrument.
You are very lucky if you can find corresponding class in :ref:`mahos.inst` package.
Even when this is not the case, writing your own :class:`Instrument <mahos.inst.instrument.Instrument>` is not very difficult; you can use existing classes in :ref:`mahos.inst` as examples.
...
Some (rather modern) instruments provide libraries for Python.
...
the easiest way to write a meas node is to subclass :class:`BasicMeasNode <mahos.meas.common_meas.BasicMeasNode>`.
We have already seen an example in :doc:`tutorial_ivcurve`.

Writing gui Node
^^^^^^^^^^^^^^^^

If a GUI is necessary, you can try writing a :class:`GUINode <mahos.gui.gui_node.GUINode>`.
Since the existing implementations are based on `PyQt <https://riverbankcomputing.com/software/pyqt/>`_,
you will need some experience or practice to use it.

If you are familiar with other UI (either native / web-based) technologies,
it is also possible to implement a GUI node with those technologies.
...
* :doc:`arch` explains the architecture of the mahos system.
* :doc:`conf` provides a more detailed specification of configuration files.
* :doc:`cli` is a list of CLI commands, but ``mahos [subcommand] -h`` may provide more information. You can also read the code in the ``mahos.cli`` package, which is not very large.

Reading the codes
^^^^^^^^^^^^^^^^^

:doc:`api` is not perfect for the time being: not everything is listed, and some classes are undocumented.
But we are working toward better documentation.
It could be used at least to discover which module or class is important.

You will eventually need to read the code to fully understand mahos internals.
We would be very happy if you could :doc:`report/fix bugs, add missing code/documentation, or give us feedback<contributing>`.
