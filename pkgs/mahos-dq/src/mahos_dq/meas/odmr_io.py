...
from mahos_dq.msgs.odmr_msgs import ODMRData, update_data
...
class ODMRIO(object):
    ...
    def save_data(self, file_name: str, data: ODMRData, note: str = "") -> bool:
        """Save data to file_name. return True on success."""

        data.set_saved()
        return save_pickle_or_h5(file_name, data, ODMRData, self.logger, note=note)

    def load_data(self, file_name: str) -> ODMRData | None:
        """Load data from file_name. return None if load is failed."""

        d = load_pickle_or_h5(file_name, ODMRData, self.logger)
        if d is not None:
    ...
    def export_data(
        self, file_name: str, data: ODMRData | list[ODMRData], params: dict | None = None
    ) -> bool:
    ...
    def _export_data_csv(self, fn, data: ODMRData) -> bool:
        ...
        with open(fn, "w", encoding="utf-8", newline="\n") as fo:
            fo.write("# ODMR data taken by mahos_dq.meas.odmr.\n")
