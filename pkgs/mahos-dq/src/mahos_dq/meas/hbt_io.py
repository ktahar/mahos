...
from mahos_dq.meas.hbt_fitter import HBTFitter
from mahos_dq.msgs.hbt_msgs import HBTData, update_data
...
class HBTIO(object):
    ...
    def save_data(self, file_name: str, data: HBTData, note: str = "") -> bool:
        """Save data to file_name. return True on success."""

        data.set_saved()
        return save_pickle_or_h5(file_name, data, HBTData, self.logger, note=note)

    def load_data(self, file_name: str) -> HBTData | None:
        """Load data from file_name. return None if load is failed."""

        d = load_pickle_or_h5(file_name, HBTData, self.logger)
        if d is not None:
    ...
    def export_data(
        self, file_name: str, data: HBTData | list[HBTData], params: dict | None = None
    ) -> bool:
    ...
    def _export_data_csv(self, fn, data: HBTData):
        ...
        with open(fn, "w", encoding="utf-8", newline="\n") as fo:
            fo.write("# HBT data taken by mahos_dq.meas.hbt.\n")
