...
from mahos_dq.msgs.spodmr_msgs import SPODMRData, update_data
from mahos_dq.meas.spodmr_worker import SPODMRDataOperator
...
class SPODMRIO(object):
    ...
    def save_data(
        self, file_name: str, data: SPODMRData, params: dict | None = None, note: str = ""
    ) -> bool:
        """Save data to file_name. return True on success."""

        data.set_saved()
        return save_pickle_or_h5(file_name, data, SPODMRData, self.logger, note=note)

    def load_data(self, file_name: str) -> SPODMRData | None:
        """Load data from file_name. return None if load is failed."""

        d = load_pickle_or_h5(file_name, SPODMRData, self.logger)
        if d is not None:
    ...
    def export_data(
        self, file_name: str, data: SPODMRData | list[SPODMRData], params: dict | None = None
    ) -> bool:
    ...
    def _export_data_csv(self, fn, data: SPODMRData) -> bool:
        header = "# Pulse ODMR data taken by mahos_dq.meas.spodmr.\n"
