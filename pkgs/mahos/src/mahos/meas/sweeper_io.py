...
class SweeperIO(object):
    ...
    def save_data(self, filename: str, data: SweeperData, note: str = "") -> bool:
        """Save data to file. Returns True on success."""

        data.set_saved()
        return save_pickle_or_h5(filename, data, SweeperData, self.logger, note=note)

    def load_data(self, filename: str) -> SweeperData | None:
        """Load data from file. Returns None on failure."""

        d = load_pickle_or_h5(filename, SweeperData, self.logger)
        if d is not None:
            return update_data(d)
        return None

    def export_data(self, filename: str, data: SweeperData, params: dict | None = None) -> bool:
        if params is None:
            params = {}
    ...
    def _export_data_csv(self, filename: str, data: SweeperData) -> bool:
    ...
    def _export_data_image(self, filename: str, data: SweeperData, params: dict) -> bool:
