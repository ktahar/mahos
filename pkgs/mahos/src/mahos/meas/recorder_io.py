...
class RecorderIO(object):
    ...
    def save_data(self, file_name: str, data: RecorderData, note: str = "") -> bool:
        """Save data to file_name. return True on success."""

        data.set_saved()
        return save_pickle_or_h5(file_name, data, RecorderData, self.logger, note=note)

    def load_data(self, file_name: str) -> RecorderData | None:
        """Load data from file_name. return None if load is failed."""

        d = load_pickle_or_h5(file_name, RecorderData, self.logger)
        if d is not None:
            return update_data(d)

    def export_data(self, file_name: str, data: RecorderData, params: dict | None = None) -> bool:
